...
# ===================== UI 스타일 =====================
st.set_page_config(page_title="천안 DRT - 스마트 교통 가이드", layout="wide")
st.markdown("""
<style>
...
with c1:
    st.caption(f"후보 정류장(추가) 개수: {len(cand_gdf)}  |  기존 정류장 개수: {len(existing_gdf)}")
...
fg_cand = folium.FeatureGroup(name="추가(후보) 정류장", show=False).add_to(m2)
